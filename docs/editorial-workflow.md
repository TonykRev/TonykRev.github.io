...
3. `technical_writer` is read-only and returns `03-draft.md`; root saves it using supported claims only.
...
Security review, site QA, and release report each require explicit decisions. A QA `PASS` needs a `# Commands` table containing the complete approved command set (`npm test`, `npm run check`, `npm run build`, `npm run test:built`, `npm run test:links`, `npm run test:e2e`, and `npm run test:lighthouse`) with integer exit-status evidence; every exit status must be `0`. Release decisions must match the security and QA artifacts, and `READY FOR HUMAN REVIEW` requires every artifact to be complete. A contract-valid run is still not release-ready: `# Publish authorization` must be exactly `NOT GRANTED` until a human separately approves publication.
